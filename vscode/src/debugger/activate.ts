  ...
  resolveDebugConfigurationWithSubstitutedVariables(
    ...
    if (config.program && folder) {
      ...
      config.programUri = folder.uri
        ...
        .toString();
      config.languageId = fileUri.path.endsWith(".qs") ? "qsharp" : "openqasm";
    } else {
      ...
      if (doc) {
        ...
        config.programUri = doc.uri.toString();
        config.languageId = doc.languageId;
        config.shots = 1;
    ...
    if (!config.programUri) {
      ...
      return vscode.window
        .showInformationMessage("Cannot find a Q# or OpenQASM program to debug")
        .then((_) => {
          return undefined;
        });
    }
  ...
  implements vscode.DebugAdapterDescriptorFactory
{
  async createDebugAdapterDescriptor(
    ...
    const program = session.configuration.languageId
      ? await getOpenQasmProgramForDocument(uri)
      : await getQSharpProgramForDocument(uri);
    if (!program.success) {
