...
function registerDocumentUpdateHandlers(
  ...
  // we manually send an OpenDocument telemetry event if this is a supported document, because the
  ...
  subscriptions.push(
    vscode.workspace.onDidCloseTextDocument((document) => {
      if (isQdkDocument(document) && !isNotebookCell2(document)) {
...
function determineDocumentType(document: vscode.TextDocument) {
  return isQdkDocument(document) && isNotebookCell2(document)
    ...
    : isCircuitDocument2(document)
      ? QsharpDocumentType.Circuit
      : isQsharpDocument2(document)
        ? QsharpDocumentType.Qsharp
        : isOpenQasmDocument2(document)
