...
describe("Testing _opToMetadata", () => {
  ...
  test("Invalid register", () => {
    ...
    expect(() => _opToMetadata(op, registers)).toThrowError(
      "ERROR: Qubit register with ID 1 not found.",
    );
    ...
    expect(() => _opToMetadata(op, registers)).toThrowError(
      "ERROR: No classical registers found for qubit ID 0.",
    );
  });
  test("skipped registers", () => {
    const op: Operation = {
...
describe("Testing _splitTargetsY", () => {
  const registers: RegisterMap = {
    ...
    2: {
      ...
      children: [
        {
          type: RegisterType.Classical,
          y: startY + registerHeight * 2 + classicalRegHeight * 2,
        },
        {
          type: RegisterType.Classical,
          y: startY + registerHeight * 2 + classicalRegHeight * 3,
        },
        {
          type: RegisterType.Classical,
          y: startY + registerHeight * 2 + classicalRegHeight * 4,
        },
      ],
    },
  };
  test("adjacent qubit regs", () => {
    ...
    expect(
      _splitTargetsY(targets, [startY + registerHeight * 3], registers),
    ).toEqual([targetsY]);
    expect(
      _splitTargetsY(targets, [0, startY + registerHeight * 3], registers),
    ).toEqual([targetsY]);
  });
  test("adjacent classical regs", () => {
    ...
    expect(
      _splitTargetsY(
        targets,
        [startY + registerHeight * 3 + classicalRegHeight * 4],
        registers,
      ),
    ).toEqual([targetsY]);
    expect(
      _splitTargetsY(
        targets,
        [0, startY + registerHeight * 3 + classicalRegHeight * 4],
        registers,
      ),
    ).toEqual([targetsY]);
  });
  ...
  test("single target", () => {
    ...
    expect(
      _splitTargetsY(targets, [startY + registerHeight], registers),
    ).toEqual([[startY]]);
    expect(
      _splitTargetsY(targets, [0, startY + registerHeight], registers),
    ).toEqual([[startY]]);
  });
  ...
  test("split two qubit regs with classical register", () => {
    ...
    expect(
      _splitTargetsY(targets, [startY + classicalRegHeight], registers),
    ).toEqual(targetsY);
    expect(
      _splitTargetsY(targets, [0, startY + classicalRegHeight], registers),
    ).toEqual(targetsY);
    expect(
      _splitTargetsY(
        targets,
        [startY + registerHeight * 2, startY + classicalRegHeight],
        registers,
      ),
    ).toEqual(targetsY);
    expect(
      _splitTargetsY(
        targets,
        [startY + registerHeight * 2, 0, startY + classicalRegHeight],
        registers,
      ),
    ).toEqual(targetsY);
  });
  test("split two classical regs with classical reg", () => {
    ...
    expect(
      _splitTargetsY(
        targets,
        [0, startY + registerHeight * 2 + classicalRegHeight * 3],
        registers,
      ),
    ).toEqual(targetsY);
    expect(
  ...
  test("split multiple targets with classical register", () => {
    ...
    expect(
      _splitTargetsY(targets, [10, startY + classicalRegHeight], registers),
    ).toEqual(targetsY);
    expect(
      _splitTargetsY(targets, [60, startY + classicalRegHeight], registers),
    ).toEqual(targetsY);
...
describe("Testing _fillMetadataX", () => {
  ...
  test("depth-1 children", () => {
    const columnWidths: number[] = Array(1).fill(
      minGateWidth + gatePadding * 2,
    );
...
describe("Testing processOperations", () => {
  test("single qubit gates", () => {
    ...
    const expectedWidth: number =
      startX + minGateWidth + rxWidth + gatePadding * 4;
  ...
  test("single wide qubit gates", () => {
    ...
    const expectedWidth: number =
      startX + minGateWidth + expectedCustomWidth + gatePadding * 4;
    const { metadataList, svgWidth } = processOperations(operations, registers);
    expect(metadataList).toEqual(expectedOps);
    expect(svgWidth).toEqual(expectedWidth);
  });
  ...
  test("grouped gates", () => {
    ...
    const expectedWidth: number =
      startX + minGateWidth * 2 + gatePadding * 4 + groupBoxPadding * 2;
    const { metadataList, svgWidth } = processOperations([op], registers);
    expect(metadataList).toEqual([metadata]);
    expect(svgWidth).toEqual(expectedWidth);
  });
  test("nested grouped gates", () => {
    ...
    const expectedWidth: number =
      startX + minGateWidth * 2 + gatePadding * 4 + groupBoxPadding * 4;
    const { metadataList, svgWidth } = processOperations([op], registers);
    expect(metadataList).toEqual([metadata]);
    expect(svgWidth).toEqual(expectedWidth);
  });
