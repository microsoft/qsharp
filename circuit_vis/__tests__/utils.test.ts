...
describe("Testing getGateWidth", () => {
  test("measure gate", () =>
    expect(
      getGateWidth(Object.assign({ type: GateType.Measure, label: "" })),
    ).toEqual(minGateWidth));
  test("cnot gate", () =>
    expect(
      getGateWidth(Object.assign({ type: GateType.Cnot, label: "x" })),
    ).toEqual(minGateWidth));
  test("swap gate", () =>
    expect(
      getGateWidth(Object.assign({ type: GateType.Swap, label: "" })),
    ).toEqual(minGateWidth));
  test("single unitary gate", () =>
    expect(
      getGateWidth(Object.assign({ type: GateType.Unitary, label: "x" })),
    ).toEqual(minGateWidth));
  test("multi unitary gate", () =>
    expect(
      getGateWidth(Object.assign({ type: GateType.Unitary, label: "zz" })),
    ).toEqual(minGateWidth));
  ...
  test("invalid", () =>
    expect(
      getGateWidth(Object.assign({ type: GateType.Invalid, label: "" })),
    ).toEqual(minGateWidth));
  test("unitary with long name", () =>
    expect(
      getGateWidth(Object.assign({ type: GateType.Unitary, label: "FOOBAR" })),
...
describe("Testing _getStringWidth", () => {
  ...
  test("varying size strings", () => {
    ...
    expect(
      _getStringWidth("GateWithASuperLongName", 14),
    ).toBeGreaterThanOrEqual(174);
    expect(_getStringWidth("GateWithASuperLongName", 14)).toBeLessThanOrEqual(
      176,
    );
  });
});
