...
export function Histogram(props: {data: Map<string, number>, filter: string, onFilter: (filter:string) => void }) {
    ...
    const barArray = [...props.data.entries()].sort( (a, b) => {
    ...
    const barOffset = 160 / barArray.length;
    const barWidth = barOffset * 0.8;
        ...
        <svg class="histogram" viewBox="0 0 165 100">
            ...
            { 
                barArray.map( (entry, idx) => {
                    const height = 72 * (entry[1] / maxCount);
                    const x = barOffset * idx;
                    const y = 87 - height;
                    const barLabel = `${entry[0]} at ${(entry[1] / totalCount * 100).toFixed(2)}%`;
                    let barClass = "bar";
