...
namespace Quantum.Kata.SingleQubitGates {
    ...
    operation PrepareArbitraryStateReference(alpha : Double, beta : Double, theta : Double, q : Qubit) : Unit is Adj + Ctl {
        body ... {
    ...
    operation VerifyTask7() : Bool {
        ...
        let taskRef = PrepareArbitraryStateReference;
        ...
        Adjoint taskRef(Cos(1.0), Sin(1.0), 2.0, target);
        ...
        DumpMachine();
        Reset(target);
        taskRef(Cos(1.0), Sin(1.0), 2.0, target);
        DumpMachine();
