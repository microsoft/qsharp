...
pub fn get_qir(program: IProgramConfig) -> Result<String, String> {
    let (source_map, capabilities, language_features, package_store) = into_qsc_args(program, None);
...
pub fn run(
    ...
    let (source_map, capabilities, language_features, package_store) =
...
pub fn generate_docs(additional_program: Option<IProgramConfig>) -> Vec<IDocFile> {
    let docs = if let Some((source_map, capabilities, language_features, package_store)) =
