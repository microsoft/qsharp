...
mod class_completions {
    ...
    #[test]
    fn primitive_class_completions() {
        check(
            r#"
            operation Foo<'T: ↘
            "#,
            // we check Iterable here but it should be `None` since we don't support generic iterables
            &["Eq", "Add", "Exp", "Integral", "Iterable", "Show", "Num" ],
            &expect![[r#"
                [
                    Some(
                        CompletionItem {
    ...
    #[test]
    fn primitive_class_composition_completions() {
        check(
            r#"
            operation Foo<'T: Eq + ↘
            "#,
