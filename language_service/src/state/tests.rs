...
#[tokio::test]
async fn notebook_defaults_to_unrestricted_not_workspace_profile() {
    let errors = RefCell::new(Vec::new());
    let test_cases = RefCell::new(Vec::new());
    let mut updater = new_updater(&errors, &test_cases);
    
    // Set workspace target profile to Base
    updater.update_configuration(WorkspaceConfigurationUpdate {
        target_profile: Some(Profile::Base),
        ..WorkspaceConfigurationUpdate::default()
    });

    // Add notebook with code that requires Unrestricted profile
    // This should NOT generate errors, even though workspace is set to Base
    updater
        .update_notebook_document(
            "notebook.ipynb",
            &NotebookMetadata::default(), // No target_profile override
            [
                (
                    "cell1",
                    1,
    ...
    // Should have no errors because notebook should default to Unrestricted
    expect_errors(&errors, &expect!["[]"]);
}

#[tokio::test]
async fn notebook_respects_explicit_target_profile_override() {
    let errors = RefCell::new(Vec::new());
    let test_cases = RefCell::new(Vec::new());
    let mut updater = new_updater(&errors, &test_cases);
    
    // Set workspace target profile to Unrestricted
    updater.update_configuration(WorkspaceConfigurationUpdate {
        target_profile: Some(Profile::Unrestricted),
        ..WorkspaceConfigurationUpdate::default()
    });

    // Add notebook with explicit Base profile override
    ...
    // Should have errors because notebook explicitly requested Base profile
    expect_errors(
        &errors,
        &expect![[r#"
            [
