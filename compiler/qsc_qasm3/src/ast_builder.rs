...
use qsc::{
    ast::{
        self, Block, Expr, ExprKind, Ident, Idents, Lit, Mutability, NodeId, Pat, PatKind, Path,
...
pub(crate) fn map_qsharp_type_to_ast_ty(output_ty: crate::types::Type) -> Ty {
    match output_ty {
        ...
        crate::types::Type::TupleArray(dims, tys) => {
            assert!(!tys.is_empty());
            let ty = map_qsharp_type_to_ast_ty(crate::types::Type::Tuple(tys));
