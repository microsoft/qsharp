...
mod qsharp {
    use super::AstDespanner;
    use qsc_codegen::qsharp::write_package_string;
    use qsc_data_structures::{language_features::LanguageFeatures, target::TargetCapabilityFlags};
    use qsc_frontend::compile::{self, compile, CompileUnit, PackageStore, SourceMap};
    use qsc_passes::{run_core_passes, run_default_passes, PackageType};
    use std::sync::Arc;

    pub(super) fn get_compilation(sources: Option<SourceMap>) -> CompileUnit {
        ...
        run_default_passes(store.core(), &mut unit, PackageType::Lib);
        unit
    }

    #[must_use]
    pub(super) fn compile_program(expr: Option<&str>, program: &str) -> (String, String) {
        ...
        let package = get_compilation(Some(sources));
...
pub(crate) fn compare_qasm_and_qasharp_asts(source: &str) {
    ...
    let package = unit.package.as_ref().expect("package must exist");
    let despanned_qasm_ast = AstDespanner.despan(package);
    ...
    let (_, despanned_qsharp_ast) = qsharp::compile_program(None, &qsharp_src);

    // 4. Compare diffs between the ASTs generated by QASM and by Q#.
    let despanned_qasm_ast = despanned_qsharp_ast.to_string();
