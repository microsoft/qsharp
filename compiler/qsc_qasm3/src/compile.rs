...
use crate::{EntrySignature, OutputSemantics, ProgramType, SemanticError, SemanticErrorKind};
...
impl QasmCompiler {
    ...
    fn build_file<S: AsRef<str>>(&mut self, name: S) -> (Package, Option<EntrySignature>) {
    ...
    fn build_operation<S: AsRef<str>>(&mut self, name: S) -> (Package, Option<EntrySignature>) {
        let tree = self.source.tree();
        let whole_span = span_for_syntax_node(tree.syntax());
    ...
    fn create_entry_item<S: AsRef<str>>(
        ...
        let mut signature = EntrySignature {
            input: vec![],
            output: String::new(),
