...
fn using_qasm_semantics_captures_all_classical_decls_as_output() -> miette::Result<(), Vec<Report>>
{
    ...
    expect![
        r#"
        namespace qasm3_import {
            operation OutputTest(theta : Double, beta : Int) : (Result[], Double, Double) {
                mutable c = [Zero, Zero];
                let q = QIR.Runtime.AllocateQubitArray(2);
                mutable gamma = 0.;
                mutable delta = 0.;
...
fn using_qiskit_semantics_only_bit_array_is_captured_and_reversed(
    ...
    expect![
        r#"
namespace qasm3_import {
    operation OutputTest(theta : Double, beta : Int) : Result[] {
        mutable c = [Zero, Zero];
        let q = QIR.Runtime.AllocateQubitArray(2);
        mutable gamma = 0.;
        mutable delta = 0.;
...
c2[2] = measure q[4];
    ...
    expect![
        r#"
namespace qasm3_import {
    operation OutputTest(theta : Double, beta : Int) : (Result[], Result[]) {
        mutable c = [Zero, Zero];
