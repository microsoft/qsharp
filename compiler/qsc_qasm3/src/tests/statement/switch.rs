...
fn spec_case_3() -> miette::Result<(), Vec<Report>> {
    ...
    expect![
        ...
        let q = QIR.Runtime.__quantum__rt__qubit_allocate();
        mutable b = [Zero, Zero];
        if Microsoft.Quantum.Convert.ResultArrayAsInt(b) == 0 {
            H(q);
        } elif Microsoft.Quantum.Convert.ResultArrayAsInt(b) == 1 {
            X(q);
        } elif Microsoft.Quantum.Convert.ResultArrayAsInt(b) == 2 {
            Y(q);
        } elif Microsoft.Quantum.Convert.ResultArrayAsInt(b) == 3 {
            Z(q);
        };
        "#
    ]
