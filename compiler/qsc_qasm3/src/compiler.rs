...
use crate::{
    ast_builder::{
        ...
        build_path_ident_expr, build_stmt_semi_from_expr_with_span, build_top_level_ns_with_item,
        build_tuple_expr, map_qsharp_type_to_ast_ty,
    },
    parser::ast::List,
...
use qsc_ast::ast::{self as qsast, Package};
...
impl QasmCompiler {
    ...
    fn compile_stmt(&mut self, stmt: &crate::semantic::ast::Stmt) -> Option<qsast::Stmt> {
        match stmt.kind.as_ref() {
            ...
            semast::StmtKind::IODeclaration(stmt) => self.compile_io_decl_stmt(stmt),
    ...
    fn compile_assign_stmt(&mut self, stmt: &semast::AssignStmt) -> Option<qsast::Stmt> {
        let (_, symbol) = self.symbols.get_symbol_by_id(stmt.symbol_id)?;
    ...
    fn compile_block_stmt(&mut self, stmt: &semast::Block) -> Option<qsast::Stmt> {
        self.push_unimplemented_error_message("block statements", stmt.span);
        None
    }
    ...
    fn compile_classical_decl(
        ...
        let (_, symbol) = self.symbols.get_symbol_by_id(decl.symbol_id)?;
        let symbol = symbol.clone();
        let name = &symbol.name;
    ...
    fn compile_io_decl_stmt(&mut self, stmt: &semast::IODeclaration) -> Option<qsast::Stmt> {
        let (_, symbol) = self.symbols.get_symbol_by_id(stmt.symbol_id)?;
    ...
    fn compile_while_stmt(&mut self, stmt: &semast::WhileLoop) -> Option<qsast::Stmt> {
        self.push_unimplemented_error_message("while statements", stmt.span);
        None
    }
    ...
    fn compile_expr(&mut self, expr: &semast::Expr) -> Option<qsast::Expr> {
        ...
        match expr.kind.as_ref() {
            ...
            semast::ExprKind::Paren(expr) => self.compile_paren_expr(expr),
        }
    }

    fn compile_ident_expr(&mut self, symbol_id: SymbolId, span: Span) -> Option<qsast::Expr> {
        let (_, symbol) = self.symbols.get_symbol_by_id(symbol_id)?;
    ...
    fn compile_indexed_ident_expr(
        ...
        let (_, symbol) = self.symbols.get_symbol_by_id(indexed_ident.symbol_id)?;
    ...
    fn compile_unary_op_expr(&mut self, unary: &UnaryOpExpr) -> Option<qsast::Expr> {
        self.push_unimplemented_error_message("unary expresssions", unary.expr.span);
        None
    }

    fn compile_binary_op_expr(&mut self, binary: &BinaryOpExpr) -> Option<qsast::Expr> {
        self.push_unimplemented_error_message("binary expressions", binary.span());
        None
    }
    ...
    fn compile_paren_expr(&mut self, paren: &Expr) -> Option<qsast::Expr> {
        self.push_unimplemented_error_message("paren expressions", paren.span);
        None
    }
