...
fn gate_call() {
    check(
        ...
        &expect![[r#"
        Stmt [0-5]
            StmtKind: QuantumStmt [0-5]: GateCall [0-5]: Ident [0-1] "H"
...
fn gate_call_qubit_register() {
    check(
        ...
        &expect![[r#"
        Stmt [0-7]
            StmtKind: QuantumStmt [0-7]: GateCall [0-7]: Ident [0-1] "H"
...
fn gate_multiple_qubits() {
    check(
        ...
        &expect![[r#"
        Stmt [0-13]
            StmtKind: QuantumStmt [0-13]: GateCall [0-13]: Ident [0-4] "CNOT"
            GateOperand IndexedIdent [5-7]: Ident [5-7] "q0"[]
            GateOperand IndexedIdent [8-12]: Ident [8-9] "q"[
            IndexElement:
                IndexSetItem Expr [10-11]: Lit: Int(4)]"#]],
    );
}
...
fn gate_no_qubits() {
    check(
        ...
        &expect![[r#"
            Stmt [0-8]
                StmtKind: QuantumStmt [0-8]: GateCall [0-8]: Ident [6-7] "H""#]],
    );
}
...
fn gate_call_with_parameters() {
    check(
        ...
        &expect![[r#"
        Stmt [0-14]
            StmtKind: QuantumStmt [0-14]: GateCall [0-14]: Ident [0-2] "Rx"
            Expr [3-9]: BinOp (Div):
...
fn gate_call_inv_modifier() {
    check(
        ...
        &expect![[r#"
            Stmt [0-11]
                StmtKind: QuantumStmt [0-11]: GateCall [0-11]: Ident [6-7] "H"
...
fn gate_call_ctrl_inv_modifiers() {
    check(
        ...
        &expect![[r#"
            Stmt [0-36]
                StmtKind: QuantumStmt [0-36]: GateCall [0-36]: Ident [16-18] "Rx"
                Expr [19-25]: BinOp (Div):
                    Expr [19-21]: Ident [19-21] "pi"
                    Expr [24-25]: Lit: Int(2)
                GateOperand IndexedIdent [27-29]: Ident [27-29] "c1"[]
                GateOperand IndexedIdent [30-32]: Ident [30-32] "c2"[]
                GateOperand IndexedIdent [33-35]: Ident [33-35] "q0"[]"#]],
    );
}
