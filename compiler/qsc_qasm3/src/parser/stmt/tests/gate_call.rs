...
fn function_call_plus_ident() {
    check(parse, "Name(2, 3) + a;", &expect![[r#"
        Stmt [0-15]
...
fn indexed_function_call() {
    check(parse, "Name(2, 3)[1];", &expect![[r#"
        Stmt [0-14]
...
fn multi_indexed_function_call() {
    check(parse, "Name(2, 3)[1, 0];", &expect![[r#"
        Stmt [0-17]
