...
fn begin_document() {
    check_valid_words(
        ...
        &expect![[r#"
            WordKinds(
                Annotation | Barrier | Box | Break | Cal | Continue | CReg | Def | DefCal | DefCalGrammar | Delay | End | Extern | False | For | Gate | If | Include | Input | Let | OpenQASM | Output | Pragma | QReg | Qubit | Reset | True | Return | Switch | While,
            )
        "#]],
    );
}
...
fn end_of_version() {
    check_valid_words(
        ...
        &expect![[r#"
            WordKinds(
                Annotation | Barrier | Box | Break | Cal | Continue | CReg | Def | DefCal | DefCalGrammar | Delay | End | Extern | False | For | Gate | If | Include | Input | Let | Output | Pragma | QReg | Qubit | Reset | True | Return | Switch | While,
            )
        "#]],
    );
}
