...
fn begin_document() {
    check_valid_words(
        ...
        &expect![[r#"
            WordKinds(
                Annotation | Continue | CReg | Def | Extern | For | Gate | If | Include | Input | OpenQASM | Output | Pragma | QReg | Qubit | Return | Switch | While,
            )
        "#]],
    );
}
...
fn end_of_version() {
    check_valid_words(
        ...
        &expect![[r#"
            WordKinds(
                Annotation | Continue | CReg | Def | Extern | For | Gate | If | Include | Input | Output | Pragma | QReg | Qubit | Return | Switch | While,
            )
        "#]],
    );
}
