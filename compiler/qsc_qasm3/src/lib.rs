...
impl QasmCompileUnit {
    ...
    #[must_use]
    pub fn into_tuple(self) -> (SourceMap, Vec<WithSource<crate::Error>>, Option<Package>) {
        (self.source_map, self.errors, self.package)
    }
}
...
where
    ...
    R: SourceResolver,
{
    // if we fail to parse, no point in continuing
    let parse_result = parse_source(source, path, resolver)?;

    // we've parsed the source, but there may be parse errors
    // preventing us from continuing, but we have a map of the
    // source and detailed errors
    if parse_result.has_errors() {
        let errors = parse_result.all_errors();
        let source_map = parse_result.source_map;
        return Ok(QasmCompileUnit {
            source_map,
            errors,
            package: None,
        });
    }
    // we have a valid parse result, so we attempt compilation
    // we need to add a semantic pass here for the qasm itself
    // along with the additional restrictions we have in what
    // qasm can be compiled.
    Ok(compile::qasm_to_program_with_semantics(
        parse_result.source,
