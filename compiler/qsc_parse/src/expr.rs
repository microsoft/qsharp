...
enum OpKind {
    ...
    Rich(fn(&mut ParserConfig, Box<Expr>) -> Result<Box<ExprKind>>),
}
...
pub(super) fn expr(s: &mut ParserConfig) -> Result<Box<Expr>> {
    expr_op(s, OpContext::Precedence(0))
}

pub(super) fn expr_eof(s: &mut ParserConfig) -> Result<Box<Expr>> {
    let expr = expr(s)?;
    token(s, TokenKind::Eof)?;
    Ok(expr)
}

pub(super) fn expr_stmt(s: &mut ParserConfig) -> Result<Box<Expr>> {
...
fn expr_op(s: &mut ParserConfig, context: OpContext) -> Result<Box<Expr>> {
...
fn expr_base(s: &mut ParserConfig) -> Result<Box<Expr>> {
    let lo = s.peek().span.lo;
...
fn expr_if(s: &mut ParserConfig) -> Result<Box<ExprKind>> {
    let cond = expr(s)?;
    let body = stmt::parse_block(s)?;
    let lo = s.peek().span.lo;
...
fn expr_set(s: &mut ParserConfig) -> Result<Box<ExprKind>> {
    let lhs = expr(s)?;
...
fn expr_array(s: &mut ParserConfig) -> Result<Box<ExprKind>> {
...
fn expr_array_core(s: &mut ParserConfig) -> Result<Box<ExprKind>> {
...
fn expr_range_prefix(s: &mut ParserConfig) -> Result<Box<ExprKind>> {
    let e = opt(s, |s| {
...
fn expr_interpolate(s: &mut ParserConfig) -> Result<Vec<StringComponent>> {
    let token = s.peek();
...
fn lit(s: &mut ParserConfig) -> Result<Option<Lit>> {
    let lexeme = s.read();
    let token = s.peek();
...
fn lambda_op(s: &mut ParserConfig, input: Expr, kind: CallableKind) -> Result<Box<ExprKind>> {
...
fn field_op(s: &mut ParserConfig, lhs: Box<Expr>) -> Result<Box<ExprKind>> {
    Ok(Box::new(ExprKind::Field(lhs, ident(s)?)))
}

fn index_op(s: &mut ParserConfig, lhs: Box<Expr>) -> Result<Box<ExprKind>> {
    let index = expr(s)?;
    token(s, TokenKind::Close(Delim::Bracket))?;
    Ok(Box::new(ExprKind::Index(lhs, index)))
}

fn call_op(s: &mut ParserConfig, lhs: Box<Expr>) -> Result<Box<ExprKind>> {
...
fn range_op(s: &mut ParserConfig, start: Box<Expr>) -> Result<Box<ExprKind>> {
...
fn op_name(s: &ParserConfig) -> OpName {
