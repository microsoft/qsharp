...
pub(super) fn ty(s: &mut ParserConfig) -> Result<Ty> {
    let lo = s.peek().span.lo;
    let lhs = base(s)?;
    array_or_arrow(s, lhs, lo)
}

pub(super) fn array_or_arrow(s: &mut ParserConfig<'_>, mut lhs: Ty, lo: u32) -> Result<Ty> {
    loop {
...
pub(super) fn param(s: &mut ParserConfig) -> Result<Box<Ident>> {
    throw_away_doc(s);
    apos_ident(s)
}

fn array(s: &mut ParserConfig) -> Result<()> {
    token(s, TokenKind::Open(Delim::Bracket))?;
    token(s, TokenKind::Close(Delim::Bracket))?;
    Ok(())
}

fn arrow(s: &mut ParserConfig) -> Result<CallableKind> {
...
fn base(s: &mut ParserConfig) -> Result<Ty> {
    throw_away_doc(s);
    let lo = s.peek().span.lo;
...
pub(super) fn functor_expr(s: &mut ParserConfig) -> Result<FunctorExpr> {
...
fn functor_base(s: &mut ParserConfig) -> Result<FunctorExpr> {
    let lo = s.peek().span.lo;
