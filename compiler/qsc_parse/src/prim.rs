...
use super::{keyword::Keyword, scan::ParserConfig, ty::ty, Error, Parser, Result};
use crate::{
    item::throw_away_doc,
    lex::{Delim, TokenKind},
    ErrorKind,
...
pub(super) fn token(s: &mut ParserConfig, t: TokenKind) -> Result<()> {
    if s.peek().kind == t {
        s.advance();
        Ok(())
    } else {
...
pub(super) fn apos_ident(s: &mut ParserConfig) -> Result<Box<Ident>> {
    let peek = s.peek();
...
pub(super) fn ident(s: &mut ParserConfig) -> Result<Box<Ident>> {
    let peek = s.peek();
...
pub(super) fn dot_ident(s: &mut ParserConfig) -> Result<Box<Ident>> {
    let p = path(s)?;
...
pub(super) fn path(s: &mut ParserConfig) -> Result<Box<Path>> {
...
pub(super) fn pat(s: &mut ParserConfig) -> Result<Box<Pat>> {
    throw_away_doc(s);
    let lo = s.peek().span.lo;
...
pub(super) fn opt<T>(s: &mut ParserConfig, mut p: impl Parser<T>) -> Result<Option<T>> {
...
pub(super) fn many<T>(s: &mut ParserConfig, mut p: impl Parser<T>) -> Result<Vec<T>> {
    let mut xs = Vec::new();
...
pub(super) fn seq<T>(s: &mut ParserConfig, mut p: impl Parser<T>) -> Result<(Vec<T>, FinalSep)>
where
    T: Default + WithSpan,
{
    let mut xs = Vec::new();
...
pub(super) fn recovering_semi(s: &mut ParserConfig) -> Result<()> {
...
pub(super) fn recovering_token(s: &mut ParserConfig, t: TokenKind) -> Result<()> {
    match token(s, t) {
        Ok(()) => Ok(()),
        Err(error) => {
            s.push_error(error);
...
fn advanced(s: &ParserConfig, from: u32) -> bool {
    s.peek().span.lo > from
}
