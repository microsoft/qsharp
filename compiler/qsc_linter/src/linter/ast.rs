...
use qsc_ast::{
    ast::{
        ...
        PathResult, QubitInit, SpecDecl, Stmt, TopLevelNode, Ty, TyDef,
    },
    visit::Visitor,
};
...
pub(crate) trait AstLintPass {
    ...
    fn check_path_result(&self, _path: &PathResult, _buffer: &mut Vec<Lint>) {}
...
macro_rules! declare_ast_lints {
    ($( ($lint_name:ident, $default_level:expr, $msg:expr, $help:expr) ),* $(,)?) => {
        ...
        mod _ast_macro_expansion {
            ...
            use qsc_ast::{
                ast::{
                    Attr, Block, CallableDecl, Expr, FunctorExpr, Ident, Item, Namespace, Package, Pat, Path, PathResult,
    ...
    (@COMBINED_STRUCT $($lint_name:ident),*) => {
        ...
        impl CombinedAstLints {
            ...
            fn check_path_result(&mut self, path: &PathResult) { $(self.$lint_name.check_path_result(path, &mut self.buffer));*; }
