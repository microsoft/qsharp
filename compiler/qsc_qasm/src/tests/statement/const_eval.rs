...
fn unary_op_neg_angle() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        import Std.Qasm.Intrinsic.*;
        let a = Std.Qasm.Angle.DoubleAsAngle(-1., 32);
        let b = Std.Qasm.Angle.AngleAsResult(a);
        mutable r = [Zero];
    "#]]
    .assert_eq(&qsharp);
    Ok(())
...
fn unary_op_negb_angle() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        import Std.Qasm.Intrinsic.*;
        let a = new Std.Qasm.Angle.Angle {
            Value = 683565276,
            Size = 32
        };
        let b = Std.Qasm.Angle.AngleAsResult(Std.Qasm.Angle.AngleNotB(a));
        mutable r = [Zero];
    "#]]
    .assert_eq(&qsharp);
    Ok(())
...
fn unary_op_negb_bitarray() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        import Std.Qasm.Intrinsic.*;
        let a = [One, Zero, One];
        let b = Std.Qasm.Convert.ResultArrayAsIntBE(~~~a);
        mutable r = [Zero, Zero];
    "#]]
    .assert_eq(&qsharp);
    Ok(())
...
fn binary_op_shl_angle() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        let b = Std.Qasm.Angle.__AngleShl__(a, 2);
        let c = Std.Qasm.Angle.AngleAsResult(b);
        mutable r = [Zero];
    "#]]
    .assert_eq(&qsharp);
    Ok(())
...
fn binary_op_shl_bit() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        import Std.Qasm.Intrinsic.*;
        let a = One;
        let b = if Std.Qasm.Convert.ResultAsInt(a) <<< 2 == 0 {
            One
        } else {
            Zero
        };
...
fn binary_op_shl_bitarray() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        import Std.Qasm.Intrinsic.*;
        let a = [One, Zero, One];
        let b = Std.Qasm.Convert.IntAsResultArrayBE(Std.Qasm.Convert.ResultArrayAsIntBE(a) <<< 2, 3);
        mutable r = [Zero, Zero, Zero, Zero];
    "#]]
    .assert_eq(&qsharp);
    Ok(())
...
fn binary_op_shr_angle() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        let b = Std.Qasm.Angle.AngleShr(a, 2);
        let c = Std.Qasm.Angle.AngleAsResult(b);
        mutable r = [Zero];
    "#]]
    .assert_eq(&qsharp);
...
fn binary_op_shr_bitarray() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        let b = Std.Qasm.Convert.IntAsResultArrayBE(Std.Qasm.Convert.ResultArrayAsIntBE(a) >>> 2, 4);
        mutable r = [Zero, Zero];
    "#]]
    .assert_eq(&qsharp);
    Ok(())
...
fn binary_op_andb_angle() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        let b = new Std.Qasm.Angle.Angle {
            Value = 1367130551,
            Size = 32
        };
        let c = Std.Qasm.Angle.AngleAndB(a, b);
        let d = Std.Qasm.Angle.AngleAsResult(c);
        mutable r = [Zero];
    "#]]
    .assert_eq(&qsharp);
    Ok(())
...
fn binary_op_andb_bit() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        import Std.Qasm.Intrinsic.*;
        let a = One;
        let b = if Std.Qasm.Convert.ResultAsInt(a) &&& 0 == 0 {
            One
        } else {
            Zero
        };
...
fn binary_op_andb_bitarray() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        import Std.Qasm.Intrinsic.*;
        let a = [One, Zero, One, One];
        let b = Std.Qasm.Convert.IntAsResultArrayBE(Std.Qasm.Convert.ResultArrayAsIntBE(a) &&& Std.Qasm.Convert.ResultArrayAsIntBE([Zero, One, One, Zero]), 4);
        mutable r = [Zero, Zero];
    "#]]
    .assert_eq(&qsharp);
    Ok(())
...
fn binary_op_orb_angle() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        let c = Std.Qasm.Angle.AngleOrB(a, b);
        let d = Std.Qasm.Angle.AngleAsBool(c);
        mutable r = [Zero];
    "#]]
    .assert_eq(&qsharp);
    Ok(())
...
fn binary_op_orb_bitarray() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        import Std.Qasm.Intrinsic.*;
        let a = [Zero, Zero, One];
        let b = Std.Qasm.Convert.IntAsResultArrayBE(Std.Qasm.Convert.ResultArrayAsIntBE(a) ||| Std.Qasm.Convert.ResultArrayAsIntBE([One, Zero, Zero]), 3);
...
fn binary_op_xorb_angle() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        let c = Std.Qasm.Angle.AngleXorB(a, b);
        let d = Std.Qasm.Angle.AngleAsResult(c);
        mutable r = [Zero];
    "#]]
    .assert_eq(&qsharp);
...
fn binary_op_xorb_bit() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        import Std.Qasm.Intrinsic.*;
        let a = One;
        let b = if Std.Qasm.Convert.ResultAsInt(a) ^^^ 1 == 0 {
            One
        } else {
            Zero
        };
...
fn binary_op_xorb_bitarray() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        import Std.Qasm.Intrinsic.*;
        let a = [One, Zero, One, One];
        let b = Std.Qasm.Convert.IntAsResultArrayBE(Std.Qasm.Convert.ResultArrayAsIntBE(a) ^^^ Std.Qasm.Convert.ResultArrayAsIntBE([One, One, One, Zero]), 4);
        mutable r = [Zero, Zero, Zero, Zero, Zero];
    "#]]
    .assert_eq(&qsharp);
    Ok(())
...
fn binary_op_add_angle() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        let c = Std.Qasm.Angle.AngleAsResult(Std.Qasm.Angle.AddAngles(a, b));
        mutable r = [Zero];
    "#]]
    .assert_eq(&qsharp);
    Ok(())
...
fn binary_op_sub_angle() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        let c = Std.Qasm.Angle.AngleAsResult(Std.Qasm.Angle.SubtractAngles(a, b));
        mutable r = [Zero];
    "#]]
    .assert_eq(&qsharp);
    Ok(())
...
fn binary_op_mul_angle() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        let b = 2;
        let c1 = Std.Qasm.Angle.AngleAsResult(Std.Qasm.Angle.MultiplyAngleByInt(a, b));
        let c2 = Std.Qasm.Angle.AngleAsResult(Std.Qasm.Angle.MultiplyAngleByInt(a, b));
        mutable r1 = [Zero];
        mutable r2 = [Zero];
    "#]]
    .assert_eq(&qsharp);
...
fn binary_op_div_angle() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        let c = 2;
        let d = if Std.Qasm.Angle.DivideAngleByAngle(Std.Qasm.Angle.AdjustAngleSizeNoTruncation(a, 48), b) == 0 {
            One
        } else {
            Zero
        };
        let e = Std.Qasm.Angle.AngleAsResult(Std.Qasm.Angle.DivideAngleByInt(a, c));
        mutable r1 = [];
        mutable r2 = [Zero];
    "#]]
    .assert_eq(&qsharp);
...
fn cast_to_bool() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        let b = 1;
        let c = 2.;
        let d = new Std.Qasm.Angle.Angle {
            Value = 1367130551,
            Size = 32
        };
        let e = One;
        ...
        let s4 = Std.Qasm.Angle.AngleAsBool(d);
        let s5 = Std.Qasm.Convert.ResultAsBool(e);
        mutable r1 = [];
        mutable r2 = [Zero];
        mutable r3 = [Zero];
        mutable r4 = [Zero];
...
fn cast_to_int() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        let c = 3.;
        let d = Zero;
        let s1 = Std.Qasm.Convert.BoolAsInt(a);
        let s2 = b;
        let s3 = Std.Math.Truncate(c);
        let s4 = Std.Qasm.Convert.ResultAsInt(d);
        mutable r1 = [Zero];
...
fn cast_to_float() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        let b = 2;
        let c = 3;
        let s1 = Std.Qasm.Convert.BoolAsDouble(a);
...
fn cast_to_angle() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        let a1 = 2.;
        let a2 = One;
        let b1 = Std.Qasm.Angle.DoubleAsAngle(a1, 32);
        let b2 = Std.Qasm.Angle.ResultAsAngle(a2);
        let s1 = Std.Qasm.Angle.AngleAsResult(b1);
        let s2 = Std.Qasm.Angle.AngleAsResult(b2);
        mutable r1 = [Zero];
        mutable r2 = [Zero];
    "#]]
    .assert_eq(&qsharp);
...
fn cast_to_bit() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        let s1 = Std.Qasm.Convert.BoolAsResult(a);
        let s2 = if b == 0 {
            One
        } else {
            Zero
        ...
        let s4 = Std.Qasm.Angle.AngleAsResult(d);
        mutable r1 = [];
        mutable r2 = [Zero];
        mutable r3 = [Zero];
        mutable r4 = [Zero];
