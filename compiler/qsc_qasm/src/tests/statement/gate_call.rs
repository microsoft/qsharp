...
fn u_gate_can_be_called_in_broadcast_qubit_array() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        U(Std.OpenQASM.Angle.DoubleAsAngle(1., 53), Std.OpenQASM.Angle.DoubleAsAngle(2., 53), Std.OpenQASM.Angle.DoubleAsAngle(3., 53), q[0]);
    "#]]
    .assert_eq(&qsharp);
    Ok(())
}
...
fn qasm2_custom_gates_can_be_called_bypassing_stdgates() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        operation u1(lambda : Std.OpenQASM.Angle.Angle, q : Qubit) : Unit is Adj + Ctl {
            U(Std.OpenQASM.Angle.DoubleAsAngle(0., 53), Std.OpenQASM.Angle.DoubleAsAngle(0., 53), lambda, q);
        }
        ...
        operation h(a : Qubit) : Unit is Adj + Ctl {
            u2(Std.OpenQASM.Angle.DoubleAsAngle(0., 53), Std.OpenQASM.Angle.DoubleAsAngle(3.141592653589793, 53), a);
        }
        operation x(a : Qubit) : Unit is Adj + Ctl {
            u3(Std.OpenQASM.Angle.DoubleAsAngle(3.141592653589793, 53), Std.OpenQASM.Angle.DoubleAsAngle(0., 53), Std.OpenQASM.Angle.DoubleAsAngle(3.141592653589793, 53), a);
        }
