...
fn single_angle_single_qubit() -> miette::Result<(), Vec<Report>> {
    ...
    let qsharp = compile_qasm_stmt_to_qsharp(source)?;
    expect![[r#"
        operation my_h(θ : Std.Qasm.Angle.Angle, q : Qubit) : Unit is Adj + Ctl {
            rx(θ, q);
        }
    "#]]
    .assert_eq(&qsharp);
...
fn two_angles_two_qubits() -> miette::Result<(), Vec<Report>> {
    ...
    let qsharp = compile_qasm_stmt_to_qsharp(source)?;
    expect![[r#"
        operation my_h(θ : Std.Qasm.Angle.Angle, φ : Std.Qasm.Angle.Angle, q : Qubit, q2 : Qubit) : Unit is Adj + Ctl {
            rx(θ, q2);
            ry(φ, q);
        }
    "#]]
