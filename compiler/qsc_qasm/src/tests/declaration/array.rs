...
fn default_simple_arrays() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        mutable b = [0, 0, 0];
        mutable c = [0, 0, 0];
        mutable d = [new Std.Qasm.Angle.Angle {
            Value = 0,
            Size = 53
        }, new Std.Qasm.Angle.Angle {
            Value = 0,
            Size = 53
        }, new Std.Qasm.Angle.Angle {
            Value = 0,
...
fn initialized_simple_arrays() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        mutable b = [-2, 0, 3];
        mutable c = [1, 2, 3];
        mutable d = [Std.Qasm.Angle.DoubleAsAngle(-1., 53), Std.Qasm.Angle.DoubleAsAngle(2., 53), Std.Qasm.Angle.DoubleAsAngle(4., 53)];
...
fn initialized_multidimensional_arrays() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        mutable c = [[0, 1, 1], [Std.Qasm.Convert.BoolAsInt(false), Std.Qasm.Convert.BoolAsInt(true), Std.Qasm.Convert.BoolAsInt(true)]];
        mutable d = [[Std.Qasm.Angle.DoubleAsAngle(-1., 53), Std.Qasm.Angle.DoubleAsAngle(0., 53), Std.Qasm.Angle.DoubleAsAngle(0., 53)], [Std.Qasm.Angle.DoubleAsAngle(1., 53), Std.Qasm.Angle.DoubleAsAngle(5., 53), Std.Qasm.Angle.DoubleAsAngle(5., 53)]];
...
fn assign_to_simple_arrays() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        set b w/= 1 <- 4;
        set c w/= 1 <- 4;
        set d w/= 1 <- new Std.Qasm.Angle.Angle {
...
fn assign_to_multidimensional_arrays() -> miette::Result<(), Vec<Report>> {
    ...
    expect![[r#"
        ...
        set b w/= 2 <- (b[2] w/ 1 <- 4);
        set c w/= 2 <- (c[2] w/ 1 <- 4);
        set d w/= 2 <- (d[2] w/ 1 <- new Std.Qasm.Angle.Angle {
            Value = 5734161139222659,
            Size = 53
        });
        set e w/= 2 <- (e[2] w/ 1 <- 4.);
