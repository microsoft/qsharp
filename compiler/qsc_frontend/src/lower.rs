        ...
        let (id, kind) = match &*item.kind {
            ...
            ast::ItemKind::ImportOrExport(item) => {
                ...
                for item in &item.items {
                    ...
                    // Fix for cross-namespace exports: if there's no alias but this is a
                    // qualified export (e.g., "export Foo.Bar;"), create an implicit alias
                    // so it behaves like "export Foo.Bar as Bar;" which already works
                    // Only apply this for user-defined cross-namespace exports, not standard library
