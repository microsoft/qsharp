...
use qsc_hir::{
    hir::{
        self, Attr, Block, CallableDecl, Expr, FunctorExpr, Ident, Item, NodeId, Package, Pat,
...
fn call_generic_identity() {
    check(
        ...
        &expect![[r##"
            ...
            #16 72-75 "Foo" : (()) -> (Int)
            #17 75-77 "()" : ()
            #19 84-99 "{ Identity(4) }" : Int
            #20 86-97 "Identity(4)" : Int
            #21 86-97 "Identity(4)" : Int
            #22 86-94 "Identity" : (Int) -> (Int)
            #23 94-97 "(4)" : Int
            #24 95-96 "4" : Int
        "##]],
    );
...
fn controlled_spec_impl() {
    check(
        ...
        "",
        &expect![[r##"
            #2 28-31 "Foo" : (Qubit) => (()) is Ctl
            #3 31-42 "(q : Qubit)" : Qubit
            #4 32-41 "q : Qubit" : Qubit
            #5 32-33 "q" : Qubit
            #10 72-75 "..." : Qubit
            #11 76-78 "{}" : ()
            #13 98-107 "(cs, ...)" : ((Qubit)[], Qubit)
            #14 99-101 "cs" : (Qubit)[]
            #15 99-101 "cs" : (Qubit)[]
            #16 103-106 "..." : Qubit
            #17 108-110 "{}" : ()
