...
type FileMap = FxHashMap<Arc<str>, (Arc<str>, Arc<str>)>;
...
pub fn generate_docs() -> FileMap {
    let compilation = Compilation::new();
    let mut file_map: FileMap = FxHashMap::default();
    ...
    generate_toc(&mut toc, &mut file_map);
...
fn generate_doc_for_item<'a>(
    ...
    file_map.insert(file_name, (file_metadata, file_content));
...
fn generate_toc(map: &mut FxHashMap<Rc<str>, Vec<String>>, file_map: &mut FileMap) {
    let header = "
