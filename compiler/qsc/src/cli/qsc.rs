...
use crate::hir::PackageId;
use crate::{compile::compile, PassContext};
...
impl From<Profile> for crate::target::Profile {
    fn from(profile: Profile) -> Self {
        match profile {
            Profile::Unrestricted => crate::target::Profile::Unrestricted,
            Profile::Base => crate::target::Profile::Base,
            Profile::AdaptiveRI => crate::target::Profile::AdaptiveRI,
        }
    }
}
...
pub fn exec(cli: Cli) -> miette::Result<ExitCode> {
    let mut store = PackageStore::new(crate::compile::core());
    let mut dependencies = Vec::new();
    let profile: crate::target::Profile = cli.profile.unwrap_or_default().into();
    ...
    if !cli.nostdlib {
        dependencies.push(store.insert(crate::compile::std(&store, capabilities)));
    }
