...
impl<'a> PartialEvaluator<'a> {
    ...
    fn generate_instructions_for_binary_operation_with_double_operands(
        ...
        let rir_variable = match bin_op {
            BinOp::Add => {
                let bin_op_variable_id = self.resource_manager.next_var();
                let bin_op_rir_variable = rir::Variable::new_double(bin_op_variable_id);
                let bin_op_rir_ins =
                    Instruction::Fadd(lhs_operand, rhs_operand, bin_op_rir_variable);
            ...
            BinOp::Sub => {
                ...
                let bin_op_rir_variable = rir::Variable::new_double(bin_op_variable_id);
                let bin_op_rir_ins =
                    Instruction::Fsub(lhs_operand, rhs_operand, bin_op_rir_variable);
                self.get_current_rir_block_mut().0.push(bin_op_rir_ins);
                bin_op_rir_variable
            }
            BinOp::Mul => {
                ...
                let bin_op_rir_variable = rir::Variable::new_double(bin_op_variable_id);
                let bin_op_rir_ins =
                    Instruction::Fmul(lhs_operand, rhs_operand, bin_op_rir_variable);
                self.get_current_rir_block_mut().0.push(bin_op_rir_ins);
                bin_op_rir_variable
            }
            BinOp::Div => {
            ...
            BinOp::Eq => {
                ...
                let bin_op_rir_ins = Instruction::Fcmp(
