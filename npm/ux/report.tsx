...
function formatThousandSep(str: string): string {
  const parts = [];
...
export function CreateReport(result: ReData): ReportData {
  ...
  entries.push({
    ...
    explanation:
      "This is a runtime estimate for the execution time of the algorithm. In general, the execution time corresponds to the duration of one logical cycle ({formatThousandSep(result.logicalQubit.logicalCycleTime)} nanosecs) multiplied by the {formatThousandSep(result.algorithmicLogicalDepth)} logical cycles to run the algorithm. If however the duration of a single T factory (here: {formatThousandSep(result.tfactory == null ? 0 result.tfactory.duration)} nanosecs) is larger than the algorithm runtime, we extend the number of logical cycles artificially in order to exceed the runtime of a single T factory.",
  });
  entries.push({
    ...
    explanation:
      "The value is computed as the number of logical qubits after layout ({formatThousandSep(result.layoutOverhead.logicalQubits)}) (with a logical error rate of {result.physcialCountFormatteds.requiredLogicalQubitErrorRate}) multiplied by the clock frequency ({formatThousandSepF64(result.logicalQubit.logicalCyclesPerSecond)}), which is the number of logical cycles per second.",
  });
  entries.push({
    ...
    explanation:
      "This value represents the total number of physical qubits, which is the sum of {formatThousandSep(result.physcialCounts.breakdown.physicalQubitsForAlgorithm)} physical qubits to implement the algorithm logic, and {formatThousandSep(result.physcialCounts.breakdown.physicalQubitsForTfactories)} physical qubits to execute the T factories that are responsible to produce the T states that are consumed by the algorithm.",
  });
  groups.push({
  ...
  entries.push({
    ...
    explanation:
      "Laying out the logical qubits in the presence of nearest-neighbor constraints requires additional logical qubits. In particular, to layout the $Q_{\rm alg} = {formatThousandSep(result.logicalCounts.numQubits)}$ logical qubits in the input algorithm, we require in total $2 cdot Q_{\rm alg} + lceil sqrt{8 cdot Q_{\rm alg}}\rceil + 1 = {formatThousandSep(result.layoutOverhead.logicalQubits)}$ logical qubits.",
  });
  entries.push({
    ...
    explanation:
      "To execute the algorithm using _Parallel Synthesis Sequential Pauli Computation_ (PSSPC), operations are scheduled in terms of multi-qubit Pauli measurements, for which assume an execution time of one logical cycle. Based on the input algorithm, we require one multi-qubit measurement for the {formatThousandSep(result.logicalCounts.measurementCount)} single-qubit measurements, the {formatThousandSep(result.logicalCounts.rotationCount)} arbitrary single-qubit rotations, and the {formatThousandSep(result.logicalCounts.tCount)} T gates, three multi-qubit measurements for each of the {formatThousandSep(result.logicalCounts.cczCount)} CCZ and {formatThousandSep(result.logicalCounts.fccixCount)} CCiX gates in the input program, as well as {result.physcialCountFormatted.numTsPerRotation} multi-qubit measurements for each of the {formatThousandSep(result.logicalCounts.rotationDepth)} non-Clifford layers in which there is at least one single-qubit rotation with an arbitrary angle rotation.",
  });
  entries.push({
    ...
    explanation:
      "This number is usually equal to the logical depth of the algorithm, which is {formatThousandSep(result.algorithmicLogicalDepth)}. However, in the case in which a single T factory is slower than the execution time of the algorithm, we adjust the logical cycle depth to exceed the T factory's execution time.",
  });
  entries.push({
    ...
    explanation:
      "This is the number of logical cycles that can be performed within one second. The logical cycle time is {result.physcialCountFormatted.logicalCycleTime}.",
  });
  entries.push({
    ...
    explanation:
      "To execute the algorithm, we require one T state for each of the {formatThousandSep(result.logicalCounts.tCount)} T gates, four T states for each of the {formatThousandSep(result.logicalCounts.cczCount)} CCZ and {formatThousandSep(result.logicalCounts.fccixCount)} CCiX gates, as well as {result.physcialCountFormatted.numTsPerRotation} for each of the {formatThousandSep(result.logicalCounts.rotationCount)} single-qubit rotation gates with arbitrary angle rotation.",
  });
  entries.push({
    path: "physicalCountsFormatted/numTfactories",
    label: "Number of T factories",
    description:
      "Number of T factories capable of producing the demanded {formatThousandSep(result.physcialCounts.breakdown.numTstates)} T states during the algorithm's runtime",
    explanation:
      "The total number of T factories {formatThousandSep(result.physcialCounts.breakdown.numTfactories)} that are executed in parallel is computed as $leftlceildfrac{\text{T states}cdot\text{T factory duration}}{\text{T states per T factory}cdot\text{algorithm runtime}}\right\rceil = leftlceildfrac{{formatThousandSep(result.physcialCounts.breakdown.numTstates)} cdot {formatThousandSep(result.tfactory == null ? 0 result.tfactory.duration)};\text{ns}}{{formatThousandSep(result.tfactory == null ? 0 result.tfactory.output_t_count)} cdot {formatThousandSep(result.physcialCounts.breakdown.runtime)};\text{ns}}\right\rceil$",
  });
  entries.push({
    ...
    explanation:
      "In order to prepare the {formatThousandSep(result.physcialCounts.breakdown.numTstates)} T states, the {formatThousandSep(result.physcialCounts.breakdown.numTfactories)} copies of the T factory are repeatedly invoked {formatThousandSep(result.physcialCounts.breakdown.numTfactoryRuns)} times.",
  });
  entries.push({
    ...
    explanation:
      "The {formatThousandSep(result.physcialCounts.breakdown.physicalQubitsForAlgorithm)} are the product of the {formatThousandSep(result.layoutOverhead.logicalQubits)} logical qubits after layout and the {formatThousandSep(result.logicalQubit.physicalQubits)} physical qubits that encode a single logical qubit.",
  });
  entries.push({
    ...
    explanation:
      "Each T factory requires {formatThousandSep(result.tfactory == null ? 0 result.tfactory.physical_qubits)} physical qubits and we run {formatThousandSep(result.physcialCounts.breakdown.numTfactories)} in parallel, therefore we need ${formatThousandSep(result.physcialCounts.breakdown.physicalQubitsForTfactories)} = {formatThousandSep(result.tfactory == null ? 0 result.tfactory.physical_qubits)} cdot {formatThousandSep(result.physcialCounts.breakdown.numTfactories)}$ qubits.",
  });
  entries.push({
    ...
    explanation:
      "The minimum logical qubit error rate is obtained by dividing the logical error probability {result.physcialCountFormatted.errorBudgetLogical} by the product of {formatThousandSep(result.layoutOverhead.logicalQubits)} logical qubits and the total cycle count {formatThousandSep(result.physcialCounts.breakdown.numCycles)}.",
  });
  entries.push({
    ...
    explanation:
      "The minimum T state error rate is obtained by dividing the T distillation error probability {result.physcialCountFormatted.errorBudgetTstates} by the total number of T states {formatThousandSep(result.physcialCounts.breakdown.numTstates)}.",
  });
  entries.push({
    ...
    explanation:
      "The number of T states to implement a rotation with an arbitrary angle is $lceil 0.53 log_2({formatThousandSep(result.logicalCounts.rotationCount)} / {result.errorBudget.rotations}) + 5.3\rceil$ [[arXiv:2203.10064](https://arxiv.org/abs/2203.10064)]. For simplicity, we use this formula for all single-qubit arbitrary angle rotations, and do not distinguish between best, worst, and average cases.",
  });
  groups.push({
  ...
  entries.push({
    ...
    explanation:
      "The code distance is the smallest odd integer greater or equal to $dfrac{2log({result.jobParams.qecScheme.crossingPrefactor} / {result.physcialCounts.breakdown.requiredLogicalQubitErrorRate})}{log({result.jobParams.qecScheme.errorCorrectionThreshold}/{result.logicalQubit.physicalQubit.cliffordErrorRate})} - 1$",
  });
  entries.push({
    ...
    explanation:
      "The number of physical qubits per logical qubit are evaluated using the formula {result.jobParams.qecScheme.physicalQubitsPerLogicalQubit} that can be user-specified.",
  });
  entries.push({
    ...
    explanation:
      "The runtime of one logical cycle in nanoseconds is evaluated using the formula {result.jobParams.qecScheme.logicalCycleTime} that can be user-specified.",
  });
  entries.push({
    ...
    explanation:
      "The logical qubit error rate is computed as ${result.jobParams.qecScheme.crossingPrefactor} cdot left(dfrac{{result.logicalQubit.physicalQubit.cliffordErrorRate}}{{result.jobParams.qecScheme.errorCorrectionThreshold}}\right)^\frac{{result.logicalQubit.codeDistance} + 1}{2}$",
  });
  entries.push({
  ...
  entries.push({
    ...
    explanation:
      "This is the formula that is used to compute the logical cycle time {formatThousandSep(result.logicalQubit.logicalCycleTime)} ns.",
  });
  entries.push({
    ...
    explanation:
      "This is the formula that is used to compute the number of physical qubits per logical qubits {formatThousandSep(result.logicalQubit.physicalQubits)}.",
  });
  groups.push({
  ...
  if (result.tfactory != null) {
    ...
    entries.push({
      ...
      explanation:
        "The T factory takes as input {formatThousandSep(result.tfactory == null ? 0 result.tfactory.input_t_count)} noisy physical T states with an error rate of {result.jobParams.qubitParams.tGateErrorRate} and produces {formatThousandSep(result.tfactory == null ? 0 result.tfactory.output_t_count)} T states with an error rate of {result.physcialCountFormatted.tstateLogicalErrorRate}.",
    });
    entries.push({
    ...
    entries.push({
      ...
      explanation:
        "This is the logical T state error rate achieved by the T factory which is equal or smaller than the required error rate {result.physcialCountFormatted.requiredLogicalTstateErrorRate}.",
    });
    groups.push({
  ...
  entries.push({
    ...
    explanation:
      "We determine {formatThousandSep(result.layoutOverhead.logicalQubits)} algorithmic logical qubits from this number by assuming to align them in a 2D grid. Auxiliary qubits are added to allow for sufficient space to execute multi-qubit Pauli measurements on all or a subset of the logical qubits.",
  });
  entries.push({
  ...
  entries.push({
    ...
    explanation:
      "This is one third of the total error budget {result.physcialCountFormatted.errorBudget} if the input algorithm contains rotation with gates with arbitrary angles, or one half of it, otherwise.",
  });
  entries.push({
  ...
  entries.push({
    ...
    explanation:
      "This is one third of the total error budget {result.physcialCountFormatted.errorBudget}.",
  });
  groups.push({
