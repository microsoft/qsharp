...
export function mapUtf16UnitsToUtf8Units(
  ...
  let utf8Index = 0;
  let posArrayIndex = 0;
  let nextUtf16Target = sorted_pos[posArrayIndex];
  for (;;) {
    ...
    if (utf16Index >= nextUtf16Target) {
      result[utf16Index] = utf8Index;
      if (++posArrayIndex >= sorted_pos.length) break;
      nextUtf16Target = sorted_pos[posArrayIndex];
    }
  ...
  if (posArrayIndex < sorted_pos.length) {
    console.error(
      `Failed to map all utf-8 positions to source locations. Remaining positions are: ${sorted_pos.slice(
        posArrayIndex
      )}`
    );
  }
