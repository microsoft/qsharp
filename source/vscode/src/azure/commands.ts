...
/**
 * Uploads supplemental input data for the job: a snippet of the source code
 * and a circuit diagram (if it can be generated).
 *
...
async function uploadSupplementalData(
  ...
  // First 1000 lines of the first source file
  const sourceSnippet = program.packageGraphSources.root.sources[0]?.[1]
    .split("\n")
    .slice(0, 1000)
    .join("\n");
  ...
  if (sourceSnippet) {
    const mimeType =
      program.projectType === "qsharp"
        ? "text/x-qsharp"
        : program.projectType === "openqasm"
          ? "text/x-qasm"
          : "text/plain"; // fallback - should not happen
