...
pub(super) fn ty_from_path(names: &Names, path: &Path) -> Ty {
    match names.get(path.id) {
        ...
        Some(
            resolve::Res::Local(_) | resolve::Res::Param { .. } | resolve::Res::Importable(..),
        ) => unreachable!(
            " A path in an expression should never resolve to a local, parameter, or as an importable, as there is syntactic differentiation."
        ),
        None => Ty::Err,
    }
}
