...
use crate::{
    compile,
    resolve::{ImportableItemKind, LocalKind, Resolver, import_item::iter_valid_items},
};
...
impl<'a> Renamer<'a> {
    ...
    fn format_res(res: &Res) -> String {
        match res {
            ...
            Res::Importable(item_kind, _) => match item_kind {
                ImportableItemKind::Callable(item_id, _) | ImportableItemKind::Ty(item_id, _) => {
...
fn compile(
    ...
    resolver.resolve_and_bind_all_namespace_imports_and_exports(&package);
...
fn import_duplicate_symbol_in_global_scope() {
    check(
        ...
        &expect![[r#"
            ...
            // DuplicateName("Baz", Span { lo: 49, hi: 52 })
        "#]],
    );
}
...
fn import_duplicate_symbol_different_source_global_scope() {
    check(
        ...
        &expect![[r#"
            ...
            // DuplicateName("Baz", Span { lo: 65, hi: 68 })
        "#]],
    );
}
...
fn disallow_importing_different_items_with_same_name() {
    check(
        ...
        &expect![[r#"
            ...
            // DuplicateName("Baz", Span { lo: 57, hi: 60 })
        "#]],
    );
}
...
#[test]
fn export_operation_shadows_namespace() {
    // operation B.A shadows namespace A when in namespace B,
    // so the export should also resolve to operation B.A without error
    check(
        indoc! {"
            namespace A {
            }
...
fn using_callable_export_as_ty() {
    check(
        ...
        &expect![[r#"
            ...
            namespace namespace6 {
                operation item7() : Unit {
                    let local35: namespace5.B = 1;
                    item1();
                }
            }

            // NotFound("D.B", Span { lo: 166, hi: 169 })
        "#]],
    );
}

#[test]
fn export_self_indirectly() {
    check(
        indoc! {"
            namespace A {
                export A.B;
