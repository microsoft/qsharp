...
pub(super) enum Error {
    ...
    #[error("namespace cannot be exported since it's a parent namespace")]
...
impl With<'_> {
    pub(super) fn lower_package(&mut self, package: &ast::Package) -> hir::Package {
        ...
        // Remove all the exports that refer to items of the same name in the
        // same namespace.
        ...
        for (id, item) in &items {
            if let hir::ItemKind::Export(name, Res::Item(original_item_id)) = &item.kind {
                if original_item_id.package.is_none() {
                    ...
                    if same_name {
                        to_export.push((id, original_item_id));
                    }
                }
            }
        }

        for (export_item_id, original_item_id) in to_export {
            ...
            match &*item.kind {
                ...
                ast::ItemKind::ImportOrExport(item) => {
                    ...
                    for item in &item.items {
                        let (Some(item_name), PathKind::Ok(item_path)) = (item.name(), &item.path)
                        else {
                            continue;
                        };

                        let res = self.path_to_res(item_path);
                        let export_item_res = self.names.get(item_name.id);
                        let local_item_id =
                            if let Some(resolve::Res::Item(export_item_id, _)) = export_item_res {
                                // If the export item is already resolved, we can use it directly.
                                export_item_id.item
                            } else {
                                unreachable!("export item {item} should have been assigned an id")
                            };

                        let mut name = self.lower_ident(item_name);
                        name.id = self.assigner.next_node();
                        let kind = hir::ItemKind::Export(name, res);
                        self.lowerer.items.push(hir::Item {
                            id: local_item_id,
                            span: item.span,
    ...
    fn path_to_res(&mut self, path: &ast::Path) -> hir::Res {
        match self.names.get(path.id) {
            ...
            Some(&resolve::Res::Importable(
                resolve::ImportableItemKind::Callable(item_id, _)
                | resolve::ImportableItemKind::Ty(item_id, _)
                | resolve::ImportableItemKind::Namespace(_, Some(item_id)),
                ..,
            )) => hir::Res::Item(item_id),
            Some(&resolve::Res::Importable(resolve::ImportableItemKind::Namespace(_, None), _)) => {
                self.lowerer
                    .errors
