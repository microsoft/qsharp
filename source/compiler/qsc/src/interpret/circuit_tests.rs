...
fn operation_with_long_gates_properly_aligned() {
    ...
    expect![[r#"
        ...
        q_1    ─ [[ ─── [Main_0] ──── H ──── X ─── Ry(1.0000) ──── X ─── ]] ─── [[ ─── [Main_0] ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── Rxx(1.0000) ────┼──── ]] ──── M ──
                                                                                           ┆                                                                                                                                       ┆          │             ╘═══
        q_2    ──────────────────────────────────────────────────────────────── [[ ─── [Main_0] ─── [[ ─── [H Rx(×3)] ──── H ─── Rx(1.0000) ─── [[ ──── [H(×2)] ──── H ─── Rx(1.0000) ──── H ─── Rx(1.0000) ─── ]] ─── ]] ─────────┆──────────┼──── ]] ─────────
                                                                                           ┆                                                                                                                                       ┆          │
...
fn operation_with_subsequent_qubits_no_double_rows() {
    ...
    expect![[r#"
        q_0    ─ [[ ─── [Main_0(1.0000)] ─── [[ ──── [Rxx(×2)(1.0000)] ─── Rxx(1.0000) ── [[ ──── [Rxx(×1)(1.0000)] ─── Rxx(1.0000) ── ]] ─── ]] ─── ]] ──
                                ┆                            ┆                  ┆                         ┆                  ┆
        q_1    ─ [[ ─── [Main_0(1.0000)] ─── [[ ──── [Rxx(×2)(1.0000)] ─── Rxx(1.0000) ── [[ ──── [Rxx(×1)(1.0000)] ─── Rxx(1.0000) ── ]] ─── ]] ─── ]] ──
    "#]]
    .assert_eq(&circ.to_string());
}
...
fn panic_in_register_grouping() {
    ...
    expect![[r#"
        q_0    ─ [[ ─── [Main_2 (q[0, 1])] ─── [[ ──── [H (q[...(×10)] ─── H (q[0]) ─── X (q[0]) ─── CX (q[0, 1]) ─── [[ ─── [H (q[0])(×9)] ─── H (q[0]) ─── X (q[0]) ─── CX (q[0, 1]) ─── H (q[0]) ─── X (q[0]) ─── CX (q[0, 1]) ─── H (q[0]) ─── X (q[0]) ─── CX (q[0, 1]) ─── H (q[0]) ─── X (q[0]) ─── CX (q[0, 1]) ─── H (q[0]) ─── X (q[0]) ─── CX (q[0, 1]) ─── H (q[0]) ─── X (q[0]) ─── CX (q[0, 1]) ─── H (q[0]) ─── X (q[0]) ─── CX (q[0, 1]) ─── H (q[0]) ─── X (q[0]) ─── CX (q[0, 1]) ─── H (q[0]) ─── X (q[0]) ─── CX (q[0, 1]) ─── ]] ─── ]] ─── ]] ──
        q_2    ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
    "#]].assert_eq(&circ.to_string());
}
...
fn dot_product_phase_estimation() {
    ...
    .expect("circuit generation should succeed");
    expect![[r#"
        q_0    ─ [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─── [[ ──── [Z Sta...(×16)] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─── [[ ─── [Z(×15)] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─── ]] ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Z Sta...(×8)] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─── [[ ──── [Z(×7)] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─── ]] ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Z Sta...(×4)] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─── [[ ──── [Z(×3)] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─── ]] ─── ]] ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Z Sta...(×2)] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─── [[ ──── [Z(×1)] ────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─── ]] ─── ]] ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [StateInitialisation_17] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ──── X ─── [[ ─── [GOracle_33] ──── H ──── X ──── H ─── ]] ──── X ─── [[ ─── [StateInitialisation_16] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ──── |0〉 ──
                                    ┆                                              │                     │                                                 │                     │                                         ┆                                      ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                     ┆                                  ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                                                                                                                                 ┆                                     ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                     ┆                                  ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                                                                                                                                                                                                              ┆                                     ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                     ┆                                  ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                                                                                                                                                                                                                                                                                           ┆                                     ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                     ┆                                  ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │                                                                                                                                                                                                                                                                                                                                                                                                                    ┆                                              │                     │                                                 │                     │                                              ┆                  │                                             ┆                                              │                     │                                                 │                     │
        q_1    ─ [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ──── [Z Sta...(×16)] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Z(×15)] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── ]] ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Z Sta...(×8)] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ──── [Z(×7)] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── ]] ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Z Sta...(×4)] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ──── [Z(×3)] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── ]] ─── ]] ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Z Sta...(×2)] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ──── [Z(×1)] ──── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── ]] ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── Z ─── [[ ─── [StateInitialisation_17] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ──── X ─── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── X ─── [[ ─── [StateInitialisation_16] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ──── |0〉 ──
                                                                                                                                                                                                                           ┆             │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                                ┆         │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                                                                                                                                                            ┆            │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                                ┆         │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                                                                                                                                                                                                                                         ┆            │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                                ┆         │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      ┆            │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                                ┆         │                                                                                                                                                                                                                    ┆                  │                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      │                                                                                                                                                                                                                    ┆                  │
        q_2    ── H ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ──── [Z Sta...(×16)] ──── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Z(×15)] ──── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ]] ─── ]] ──── H ──── M ──── |0〉 ──── H ─── [[ ──── [check (a = |1〉)] ─── [[ ─── [true] ─── Rz(-1.5708) ── ]] ─── ]] ─── [[ ─── [Z Sta...(×8)] ──── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ──── [Z(×7)] ──── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ]] ─── ]] ──── H ──── M ──── |0〉 ──── H ─── [[ ──── [check (a = |1〉)] ─── [[ ─── [true] ─── Rz(-0.7854) ── ]] ─── ]] ─── [[ ──── [check (a = |1〉)] ─── [[ ─── [true] ─── Rz(-1.5708) ── ]] ─── ]] ─── [[ ─── [Z Sta...(×4)] ──── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ──── [Z(×3)] ──── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ]] ─── ]] ──── H ──── M ──── |0〉 ──── H ─── [[ ──── [check (a = |1〉)] ─── [[ ─── [true] ─── Rz(-0.3927) ── ]] ─── ]] ─── [[ ──── [check (a = |1〉)] ─── [[ ─── [true] ─── Rz(-0.7854) ── ]] ─── ]] ─── [[ ──── [check (a = |1〉)] ─── [[ ─── [true] ─── Rz(-1.5708) ── ]] ─── ]] ─── [[ ─── [Z Sta...(×2)] ──── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ──── [Z(×1)] ──── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ]] ─── ]] ──── H ──── M ──── |0〉 ──── H ─── [[ ──── [check (a = |1〉)] ─── [[ ─── [true] ─── Rz(-0.1963) ── ]] ─── ]] ─── [[ ──── [check (a = |1〉)] ─── [[ ─── [true] ─── Rz(-0.3927) ── ]] ─── ]] ─── [[ ──── [check (a = |1〉)] ─── [[ ─── [true] ─── Rz(-0.7854) ── ]] ─── ]] ─── [[ ──── [check (a = |1〉)] ─── [[ ─── [true] ─── Rz(-1.5708) ── ]] ─── ]] ──── ● ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [GOracle_33] ─────────── ● ────────── ]] ──── H ──── M ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── |0〉 ──
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  ╘════════════════════════════════════ ● ═════════════════════ ● ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪════════════════════════════════════ ● ═════════════════════ ● ═══════════════════════════════════════════════════╪═══════════════════════╪════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪════════════════════════════════════ ● ═════════════════════ ● ═══════════════════════════════════════════════════╪═══════════════════════╪════════════════════════════════════════════════════╪═══════════════════════╪══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪════════════════════════════════════ ● ═════════════════════ ● ═══════════════════════════════════════════════════╪═══════════════════════╪════════════════════════════════════════════════════╪═══════════════════════╪════════════════════════════════════════════════════╪═══════════════════════╪═════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 ╘═════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ═════════════════════ ● ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪═════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ═════════════════════ ● ═══════════════════════════════════════════════════╪═══════════════════════╪══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪═════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ═════════════════════ ● ═══════════════════════════════════════════════════╪═══════════════════════╪════════════════════════════════════════════════════╪═══════════════════════╪═════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 ╘══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ═════════════════════ ● ═════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ═════════════════════ ● ═══════════════════════════════════════════════════╪═══════════════════════╪═════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                ╘═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ═════════════════════ ● ════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
...
fn dynamics_small() {
    ...
    .expect("circuit generation should succeed");
    expect![[r#"
        q_0    ─ Rx(0.3730) ─── [[ ─── [ApplyDoubleZ_22(0.7461)] ───────────────── Rzz(0.7461) ── Rzz(0.7461) ── ]] ─── Rx(0.7461) ─── [[ ─── [ApplyDoubleZ_22(0.7461)] ───────────────── Rzz(0.7461) ── Rzz(0.7461) ── ]] ─── Rx(-0.2191) ── [[ ─── [ApplyDoubleZ_22(0.5922)] ───────────────── Rzz(0.5922) ── Rzz(0.5922) ── ]] ─── Rx(-0.2191) ── [[ ─── [Apply...(×3)(0.7461)] ─── [[ ─── [ApplyDoubleZ_22(0.7461)] ───────────────── Rzz(0.7461) ── Rzz(0.7461) ── ]] ─── Rx(0.7461) ─── [[ ──── [ApplyDoubleZ_22(×2)(0.7461)] ─── [[ ─── [ApplyDoubleZ_22(0.7461)] ───────────────── Rzz(0.7461) ── Rzz(0.7461) ── ]] ─── Rx(0.7461) ─── [[ ─── [ApplyDoubleZ_22(0.7461)] ───────────────── Rzz(0.7461) ── Rzz(0.7461) ── ]] ─── Rx(0.7461) ─── ]] ─── ]] ─── [[ ─── [ApplyDoubleZ_22(0.7461)] ───────────────── Rzz(0.7461) ── Rzz(0.7461) ── ]] ─── Rx(-0.2191) ── [[ ─── [ApplyDoubleZ_22(0.5922)] ───────────────── Rzz(0.5922) ── Rzz(0.5922) ── ]] ─── Rx(-0.2191) ── [[ ─── [ApplyDoubleZ_22(0.7461)] ───────────────── Rzz(0.7461) ── Rzz(0.7461) ── ]] ─── Rx(0.7461) ─── [[ ─── [ApplyDoubleZ_22(0.7461)] ───────────────── Rzz(0.7461) ── Rzz(0.7461) ── ]] ─── Rx(0.3730) ──
                                                   ┆                                    ┆              ┆                                                  ┆                                    ┆              ┆                                                  ┆                                    ┆              ┆                                                 ┆                                  ┆                                    ┆              ┆                                                     ┆                                      ┆                                    ┆              ┆                                                  ┆                                    ┆              ┆                                                                ┆                                    ┆              ┆                                                  ┆                                    ┆              ┆                                                  ┆                                    ┆              ┆                                                  ┆                                    ┆              ┆
