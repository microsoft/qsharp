...
def check_prereqs(install=False):
    ...
    if version_match:
        ...
        if rustfmt_major < rust_fmt_ver[0] or (rustfmt_major == rust_fmt_ver[0] and rustfmt_minor < rust_fmt_ver[1]):
            print(f'cargo fmt v{rust_fmt_ver[0]}.{rust_fmt_ver[1]} or later is required. Please update')
            exit(1)
    else:
    ...
    if version_match:
        ...
        if (clippy_major < clippy_ver[0] or (clippy_major == clippy_ver[0] and clippy_minor < clippy_ver[1]) or
                (clippy_major == clippy_ver[0] and clippy_minor == clippy_ver[1] and clippy_patch < clippy_ver[2])):
            print(f'clippy v{clippy_ver[0]}.{clippy_ver[1]}.{clippy_ver[2]} or later is required. Please update')
            exit(1)
    else:
