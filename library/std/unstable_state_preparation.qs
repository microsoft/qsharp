namespace Microsoft.Quantum.Unstable.StatePreparation {
    ...
    function ComputePreparationParameters(
        targetError : Double,
        coefficients : Double[],
        data: Bool[][]) : PreparationParameters {
    ...
    operation PreareQuantumState(
        parameters: PreparationParameters,
        selectOperation: (Bool[][], Qubit[], Qubit[]) => Unit is Adj + Ctl,
        indexRegister: Qubit[],
        dataQubits: Qubit[],
        garbageRegister: Qubit[]) : Unit is Adj + Ctl {

        PrepareQuantumROMState(
            parameters::nBitsPrecision,
            parameters::nCoeffs,
            parameters::nBitsIndices,
            parameters::keepCoeff,
            parameters::altIndex,
            parameters::data,
            selectOperation,
            indexRegister,
    ...
    // ----------------------------------- //
    // State preparation (test operations) //
    // ----------------------------------- //

    operation TestPrepareUniform(numStates : Int, numQubits : Int) : Unit {
        use qs = Qubit[numQubits];
        PrepareUniformSuperposition(numStates, qs);

        DumpMachine();
        ResetAll(qs);
    }

    operation TestPrepareWithCoefficients(precision : Double, coefficients : Double[], flagIndex : Int) : Unit {
        let state = MakePrepareArbitrarySuperposition(precision, coefficients);

        use index = Qubit[state::NIndexQubits];
        use garbage = Qubit[state::NGarbageQubits];
        use flag = Qubit();

        Message($"!flag = {flag}");

        within {
            state::Prepare(index, [], garbage);
            ApplyControlledOnInt(flagIndex, X, index, flag);
        } apply {
            DumpMachine();
        }

        ResetAll(index + garbage + [flag]);
    }

    operation TestPrepareWithCoefficientsAndData(precision : Double, coefficients : Double[], data : Bool[][], flagIndex : Int) : Unit {
        let state = MakePrepareArbitrarySuperpositionWithData(precision, coefficients, data);

        use index = Qubit[state::NIndexQubits];
        use garbage = Qubit[state::NGarbageQubits];
        use dataRegister = Qubit[Length(data[0])];
        use flag = Qubit();

        Message($"!flag = {flag}");

        within {
            state::Prepare(index, dataRegister, garbage);
            ApplyControlledOnBitString(data[flagIndex], X, dataRegister, flag);
        } apply {
            DumpMachine();
        }
