...
=======
        ...
        for (list, expectedMinIndex) in Zipped(testLists, expectedMinIndices) {
            ...
            // Assert that the probability is above 50%
            Fact(probability > 0.5, $"Probability of finding the minimum for list {list} is less than 50%. Found: {probability * 100.0}%");
...
=======
        ...
        let expectedMinIndices : Int[] = [0, 0, 0];
>>>>>>> Stashed changes
...
=======
            ...
            // Run the Durr-Hoyer algorithm multiple times (shots)
            for _ in 1..shots {
                let minIndex : Int = DurrHoyerAlgorithm(list, nQubits, "max");
